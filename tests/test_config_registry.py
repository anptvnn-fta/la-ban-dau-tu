...
class TestSettingsHelpContract(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        # The legacy zh/en frontend (apps/dsa-web) was dropped in the VN rebuild;
        # the new Vietnamese frontend uses a different strings layout, so this
        # locale-contract check is skipped when the legacy help file is absent.
        if not cls._SETTINGS_HELP_FILE.exists():
            raise unittest.SkipTest("legacy settingsHelp.ts not present (VN frontend rebuild)")
