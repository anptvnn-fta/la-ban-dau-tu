...
"""
===================================
Lớp cơ sở nguồn dữ liệu và bộ quản lý
===================================

Mẫu thiết kế: Strategy Pattern (Chiến lược)
- BaseFetcher: lớp trừu tượng, định nghĩa giao diện thống nhất
- DataFetcherManager: bộ quản lý chiến lược, thực hiện chuyển đổi tự động

Chiến lược chống bị chặn:
1. Mỗi Fetcher tích hợp logic kiểm soát lưu lượng
2. Tự động chuyển sang nguồn dữ liệu tiếp theo khi thất bại
3. Cơ chế thử lại với exponential backoff
"""
...
from .vnstock_fundamental_adapter import VnstockFundamentalAdapter
...
def normalize_stock_code(stock_code: str) -> str:
    ...
    if '.' in code:
        ...
        if suffix.upper() in ('KS', 'KQ') and base.isdigit() and len(base) == 6:
            return f"{base}.{suffix.upper()}"
        # Vietnam: keep canonical uppercase .VN suffix form (e.g. fpt.vn -> FPT.VN)
        if suffix.upper() == 'VN' and base.isalpha() and 2 <= len(base) <= 3:
            return f"{base.upper()}.VN"
...
def _is_us_market(code: str) -> bool:
    """Kiểm tra xem có phải mã cổ phiếu/chỉ số Mỹ hay không (không bao gồm tiền/hậu tố tiếng Trung)."""
...
def _is_hk_market(code: str) -> bool:
    """
    Xác định xem có phải mã cổ phiếu Hong Kong hay không.

    Hỗ trợ dạng `HK00700` và dạng 5 chữ số thuần (A-share ETF/cổ phiếu thường là 6 chữ số).
    """
    normalized = (code or "").strip().upper()
...
def _is_jp_market(code: str) -> bool:
    """Xác định xem có phải mã suffix Yahoo Finance Nhật Bản hay không (ví dụ 7203.T)."""
    normalized = (code or "").strip().upper()
...
def _is_kr_market(code: str) -> bool:
    """Xác định xem có phải mã suffix Yahoo Finance Hàn Quốc hay không (ví dụ 005930.KS / 035720.KQ)."""
    normalized = (code or "").strip().upper()
...
def _is_vn_market(code: str) -> bool:
    """Xác định xem có phải cổ phiếu Việt Nam hay không (bắt buộc có hậu tố .VN, ví dụ FPT.VN / VCB.VN).

    Mã cổ phiếu Việt Nam gồm 2-3 chữ cái viết hoa + hậu tố .VN. Mã trần (FPT, GAS, BID) có thể
    trùng với mã cổ phiếu Mỹ nên không nhận dạng ở đây — bắt buộc phải có hậu tố .VN.
    """
    normalized = (code or "").strip().upper()
    if not normalized.endswith(".VN"):
        return False
    base = normalized[:-3]
    return bool(base) and base.isalpha() and 2 <= len(base) <= 3


def _is_etf_code(code: str) -> bool:
    """Xác định mã ETF quỹ A-share (quy tắc bảo thủ)."""
...
def _market_tag(code: str) -> str:
    """Trả về nhãn thị trường: cn/us/hk/jp/kr/vn."""
    if _is_vn_market(code):    # Ưu tiên nhận diện hậu tố .VN của Việt Nam, tránh bị regex cổ phiếu Mỹ bắt nhầm
        return "vn"
...
class DataFetchError(Exception):
    """Lớp ngoại lệ cơ sở cho lỗi lấy dữ liệu"""
    pass


class RateLimitError(DataFetchError):
    """Ngoại lệ giới hạn tốc độ API"""
    pass


class DataSourceUnavailableError(DataFetchError):
    """Ngoại lệ nguồn dữ liệu không khả dụng"""
    pass


class BaseFetcher(ABC):
    """
    Lớp trừu tượng cơ sở cho nguồn dữ liệu

    Trách nhiệm:
    1. Định nghĩa giao diện lấy dữ liệu thống nhất
    2. Cung cấp phương thức chuẩn hóa dữ liệu
    3. Thực hiện tính toán chỉ báo kỹ thuật chung

    Các lớp con cần triển khai:
    - _fetch_raw_data(): lấy dữ liệu thô từ nguồn dữ liệu cụ thể
    - _normalize_data(): chuyển đổi dữ liệu thô sang định dạng chuẩn
    """
    
    name: str = "BaseFetcher"
    priority: int = 99  # Số ưu tiên càng nhỏ càng được ưu tiên cao hơn
    allow_empty_daily_data: bool = False
    
    @abstractmethod
    def _fetch_raw_data(self, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        Lấy dữ liệu thô từ nguồn dữ liệu (lớp con bắt buộc phải triển khai)

        Args:
            stock_code: Mã cổ phiếu, ví dụ '600519', '000001'
            start_date: Ngày bắt đầu, định dạng 'YYYY-MM-DD'
            end_date: Ngày kết thúc, định dạng 'YYYY-MM-DD'

        Returns:
            DataFrame dữ liệu thô (tên cột tùy theo nguồn dữ liệu)
        """
        pass
    ...
    def _normalize_data(self, df: pd.DataFrame, stock_code: str) -> pd.DataFrame:
        """
        Chuẩn hóa tên cột dữ liệu (lớp con bắt buộc phải triển khai)

        Thống nhất tên cột từ các nguồn dữ liệu khác nhau về:
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[List[Dict[str, Any]]]:
        """
        Lấy dữ liệu thời gian thực của các chỉ số chính

        Args:
            region: Khu vực thị trường, cn=A-share, us=Mỹ

        Returns:
            List[Dict]: Danh sách chỉ số, mỗi phần tử là dict gồm:
                - code: Mã chỉ số
                - name: Tên chỉ số
                - current: Điểm hiện tại
                - change: Điểm tăng/giảm
                - change_pct: Biên độ tăng/giảm (%)
    ...
    def get_market_stats(self) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dict: Gồm:
                - up_count: Số mã tăng giá
                - down_count: Số mã giảm giá
                - flat_count: Số mã đứng giá
                - limit_up_count: Số mã trần
                - limit_down_count: Số mã sàn
                - total_amount: Tổng giá trị giao dịch hai sàn
        """
        return None

    def get_sector_rankings(self, n: int = 5) -> Optional[Tuple[List[Dict], List[Dict]]]:
        """
        Lấy bảng xếp hạng tăng/giảm theo ngành

        Args:
            n: Số lượng kết quả trả về hàng đầu

        Returns:
            Tuple: (danh sách ngành tăng mạnh nhất, danh sách ngành giảm mạnh nhất)
        """
        return None

    def get_concept_rankings(self, n: int = 5) -> Optional[Tuple[List[Dict], List[Dict]]]:
        """
        Lấy bảng xếp hạng tăng/giảm theo nhóm/chủ đề.

        Returns:
            Tuple: (danh sách nhóm tăng mạnh nhất, danh sách nhóm giảm mạnh nhất)
        """
        return None

    def get_hot_stocks(self, n: int = 10) -> Optional[List[Dict[str, Any]]]:
        """
        Lấy bảng cổ phiếu được quan tâm nhiều nhất thị trường.

        Returns:
            List[Dict]: Danh sách cổ phiếu được quan tâm
        """
        return None
    ...
    def get_limit_up_pool(
        ...
        """
        Lấy danh sách cổ phiếu trần/chuỗi trần liên tiếp.

        Args:
            date: YYYYMMDD, mặc định do nguồn dữ liệu cụ thể quyết định
            n: Số bản ghi trả về
        """
        return None
    ...
    def get_daily_data(
        ...
        """
        Lấy dữ liệu ngày (điểm vào thống nhất)

        Quy trình:
        1. Tính khoảng ngày
        2. Gọi lớp con lấy dữ liệu thô
        3. Chuẩn hóa tên cột
        ...
        Args:
            ...
            end_date: Ngày kết thúc (tùy chọn, mặc định hôm nay)
            days: Số ngày lấy dữ liệu (dùng khi start_date không được chỉ định)

        Returns:
            DataFrame đã chuẩn hóa, bao gồm các chỉ báo kỹ thuật
        """
        # Tính khoảng ngày
        if end_date is None:
        ...
        if start_date is None:
            # Mặc định lấy khoảng 30 ngày giao dịch gần nhất (ước tính theo ngày lịch, lấy thêm một ít)
        ...
        try:
            ...
            # Step 4: Tính toán chỉ báo kỹ thuật
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Xử lý:
        1. Đảm bảo định dạng cột ngày chính xác
        2. Chuyển đổi kiểu dữ liệu số
        3. Loại bỏ các hàng có giá trị rỗng
        4. Sắp xếp theo ngày
        """
        df = df.copy()
        ...
        # Loại bỏ các hàng có cột quan trọng bị rỗng
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tinh toan chi bao ky thuat

        Cac chi bao duoc tinh:
        - MA5, MA10, MA20: Duong trung binh dong
        - Volume_Ratio: Ty le khoi luong (KLGD hom nay / KLGD trung binh 5 ngay)
        """
        df = df.copy()
        ...
        # Tỷ lệ khối lượng: KLGD ngày / KLGD trung bình 5 ngày
        # Lưu ý: volume_ratio ở đây là “KLGD ngày / KLGD trung bình 5 ngày trước (shift 1)”,
        # khác với “tỷ lệ khối lượng phân kỳ (cùng thời điểm)” trong một số phần mềm giao dịch,
        # ý nghĩa gần hơn với “hệ số phóng lượng”. Hành vi này được giữ nguyên theo yêu cầu.
    ...
    def random_sleep(min_seconds: float = 1.0, max_seconds: float = 3.0) -> None:
        """
        Ngủ ngẫu nhiên thông minh (Jitter)

        Chiến lược chống bị chặn: mô phỏng độ trễ ngẫu nhiên như hành vi con người,
        thêm thời gian chờ không đều giữa các yêu cầu.
        """
...
class DataFetcherManager:
    """
    Bộ quản lý chiến lược nguồn dữ liệu

    Trách nhiệm:
    1. Quản lý nhiều nguồn dữ liệu (sắp xếp theo độ ưu tiên)
    2. Tự động chuyển đổi khi có sự cố (Failover)
    3. Cung cấp giao diện lấy dữ liệu thống nhất

    Chiến lược chuyển đổi:
    - Ưu tiên sử dụng nguồn dữ liệu có độ ưu tiên cao nhất
    - Tự động chuyển sang nguồn tiếp theo khi thất bại
    - Ném ngoại lệ khi tất cả nguồn dữ liệu đều thất bại
    """
    ...
    def __init__(self, fetchers: Optional[List[BaseFetcher]] = None):
        ...
        Args:
            fetchers: Danh sách nguồn dữ liệu (tùy chọn, mặc định tự động tạo theo độ ưu tiên)
        """
        ...
        if fetchers:
            ...
            # Các nguồn dữ liệu mặc định sẽ được tải lazy khi lần đầu sử dụng
            self._init_default_fetchers()
        self._fundamental_adapter = AkshareFundamentalAdapter()
        self._yfinance_fundamental_adapter = YfinanceFundamentalAdapter()
        try:
            self._vnstock_fundamental_adapter = VnstockFundamentalAdapter()
        except Exception:  # noqa: BLE001 - vnstock optional
            self._vnstock_fundamental_adapter = None
    ...
    def _get_fundamental_cache_key(self, stock_code: str, budget_seconds: Optional[float] = None) -> str:
        """Tạo cache key cho dữ liệu cơ bản (bao gồm phân nhóm ngân sách để tránh kết quả ngân sách thấp làm ô nhiễm yêu cầu ngân sách cao)."""
    ...
    def _init_default_fetchers(self) -> None:
        """
        Khởi tạo danh sách nguồn dữ liệu mặc định

        Logic điều chỉnh độ ưu tiên động:
        - Nếu đã cấu hình TUSHARE_TOKEN: khởi tạo TushareFetcher và nâng độ ưu tiên theo logic nội bộ
        - Nếu đã cấu hình Longbridge OAuth hoặc Legacy credentials: khởi tạo LongbridgeFetcher làm dự phòng Mỹ/HK
        - Các nguồn tùy chọn chưa cấu hình sẽ không khởi tạo, tránh thăm dò vô ích khi lấy dữ liệu hàng loạt
        - Độ ưu tiên mặc định:
          0. EfinanceFetcher (Priority 0) - ưu tiên cao nhất
        ...
        config = get_config()
        # Tạo tất cả các instance nguồn dữ liệu (độ ưu tiên được xác định trong __init__ của từng Fetcher)
        efinance = EfinanceFetcher()
        tencent = TencentFetcher()
        akshare = AkshareFetcher()
        pytdx = PytdxFetcher()      # Nguồn dữ liệu Tongdaxin (có thể cấu hình PYTDX_HOST/PYTDX_PORT)
        ...
        if tushare_token:
            optional_fetchers.append(TushareFetcher())  # Tự động điều chỉnh độ ưu tiên theo cấu hình Token
        else:
            logger.debug("[数据源初始化] 跳过未配置的 TushareFetcher")

        if LongbridgeFetcher.has_configured_credentials(config):
            optional_fetchers.append(LongbridgeFetcher())  # Longbridge (dự phòng Mỹ/HK, lazy load)
        else:
        ...
        # Nguồn dữ liệu thị trường Việt Nam (vnstock). Tùy chọn: bỏ qua khi vnstock chưa cài, không ảnh hưởng thị trường khác.
        try:
            from .vnstock_fetcher import VnstockFetcher
            optional_fetchers.append(VnstockFetcher())
        except Exception as exc:  # noqa: BLE001
            logger.debug(f"[数据源初始化] 跳过 VnstockFetcher（vnstock 未安装或不可用）: {exc}")

        # Khởi tạo danh sách nguồn dữ liệu
        self._ensure_concurrency_guards()
        with self._fetchers_lock:
            self._fetchers = [
                efinance,
            ...
            # Sắp xếp theo độ ưu tiên (Tushare có độ ưu tiên 0 nếu cấu hình Token thành công)
            self._fetchers.sort(key=lambda f: f.priority)
            self._refresh_fetcher_indexes_locked()

        # Tạo thông tin mô tả độ ưu tiên
    ...
    def add_fetcher(self, fetcher: BaseFetcher) -> None:
        """Thêm nguồn dữ liệu và sắp xếp lại"""
        self._ensure_concurrency_guards()
        with self._fetchers_lock:
    ...
    def get_daily_data(
        ...
        """
        Lấy dữ liệu ngày (tự động chuyển đổi nguồn dữ liệu)

        Chiến lược failover:
        1. Chỉ số/cổ phiếu Mỹ được định tuyến trực tiếp đến YfinanceFetcher
        2. Các mã khác bắt đầu thử từ nguồn dữ liệu ưu tiên cao nhất
        3. Tự động chuyển sang nguồn tiếp theo sau khi bắt ngoại lệ
        4. Ghi lại lý do thất bại của từng nguồn dữ liệu
        5. Ném ngoại lệ chi tiết khi tất cả nguồn đều thất bại
        ...
        Returns:
            Tuple[DataFrame, str]: (dữ liệu, tên nguồn dữ liệu thành công)

        Raises:
            DataFetchError: Ném khi tất cả nguồn dữ liệu đều thất bại
        """
        ...
        # Đường nhanh: cổ phiếu Mỹ dùng định tuyến nguồn chuyên dụng; HK lọc trước các nguồn không hỗ trợ dữ liệu ngày HK
        #   - Sau khi cấu hình Longbridge: Longbridge là ưu tiên, YFinance/AkShare là dự phòng
        #   - Chưa cấu hình Longbridge:   YFinance là ưu tiên (Mỹ), vòng lặp fetcher chung (HK)
        #   - Chỉ số Mỹ:                  luôn ưu tiên YFinance (Longbridge không cung cấp K-line chỉ số)
        ...
        if total_fetchers == 0:
            market_label = "美股指数" if is_us_index else "美股" if is_us else "港股" if is_hk else "越南股" if is_vn else "A股"
            error_summary = f"{market_label} {stock_code} 获取失败:\n暂无可用数据源"
            logger.error(f"[数据源终止] {stock_code} 获取失败: {error_summary}")
            raise DataFetchError(error_summary)

        # Cổ phiếu Mỹ (bao gồm chỉ số Mỹ) dùng định tuyến chuyên dụng; HK đi vào vòng lặp nguồn chung bên dưới
        ...
        if is_us:
            ...
            if is_us_index:
                # Chỉ số luôn ưu tiên YFinance (Longbridge không cung cấp K-line chỉ số)
        ...
        for attempt, fetcher in enumerate(fetchers, start=1):
            ...
            try:
                ...
                # Tiếp tục thử nguồn dữ liệu tiếp theo
                continue

        # Tất cả nguồn dữ liệu đều thất bại
    ...
    def available_fetchers(self) -> List[str]:
        """Trả về danh sách tên các nguồn dữ liệu khả dụng"""
        return [f.name for f in self._get_fetchers_snapshot()]
    
    def prefetch_realtime_quotes(self, stock_codes: List[str]) -> int:
        """
        Tải trước hàng loạt dữ liệu thời gian thực (gọi trước khi bắt đầu phân tích)

        Chiến lược:
        1. Kiểm tra xem nguồn dữ liệu lấy toàn bộ thị trường (efinance/akshare_em) có trong danh sách ưu tiên không
        2. Nếu không có, bỏ qua tải trước (Sina/Tencent là truy vấn từng cổ phiếu, không cần tải trước)
        3. Nếu số lượng cổ phiếu theo dõi >= 5 và dùng nguồn toàn thị trường, tải trước để điền cache

        Lợi ích:
        - Khi dùng Sina/Tencent: mỗi cổ phiếu truy vấn độc lập, không có vấn đề tải toàn thị trường
        - Khi dùng efinance/Eastmoney: tải trước một lần, các lần sau cache hit

        Args:
            stock_codes: Danh sách mã cổ phiếu cần phân tích

        Returns:
            Số lượng cổ phiếu được tải trước (0 nghĩa là bỏ qua tải trước)
        """
        # Normalize all codes
        ...
        # Issue #455: Có thể tắt tải trước bằng PREFETCH_REALTIME_QUOTES=false, tránh kéo toàn thị trường
        ...
        # Nếu tính năng thời gian thực bị tắt, bỏ qua tải trước
        if not config.enable_realtime_quote:
            logger.debug("[预取] component=realtime_prefetch action=skip reason=realtime_quote_disabled")
            return 0
        
        # Kiểm tra xem trong danh sách ưu tiên có nguồn dữ liệu toàn thị trường không
        # Lưu ý: cần cập nhật danh sách này khi thêm API toàn thị trường mới (ví dụ tushare_realtime)
        # Đặc điểm API toàn thị trường: một lần gọi API kéo dữ liệu 5000+ cổ phiếu toàn thị trường
        priority = config.realtime_source_priority.lower()
        bulk_sources = ['efinance', 'akshare_em', 'tushare']  # Danh sách API toàn thị trường

        # Nếu cả hai nguồn đầu trong danh sách ưu tiên đều không phải nguồn toàn thị trường, bỏ qua tải trước
        # Vì Sina/Tencent là truy vấn từng cổ phiếu, không cần tải trước
        ...
        # Nếu không có nguồn toàn thị trường, hoặc nguồn toàn thị trường xếp sau vị trí thứ 3, bỏ qua tải trước
        ...
        # Nếu số lượng cổ phiếu ít hơn 5, không tải trước hàng loạt (truy vấn từng cái hiệu quả hơn)
        ...
        # Thử tải trước qua efinance hoặc akshare
        # Chỉ cần gọi get_realtime_quote một lần, cơ chế cache sẽ tự động kéo dữ liệu toàn thị trường
        try:
            # Dùng cổ phiếu đầu tiên để kích hoạt tải toàn thị trường
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        # Nếu tính năng thời gian thực bị tắt, trả về None ngay
        if not config.enable_realtime_quote:
            logger.debug(f"[实时行情] 功能已禁用，跳过 {stock_code}")
            return None

        # ----------------------------------------------------------
        # Thị trường Việt Nam (hậu tố .VN) — định tuyến chuyên dụng: chỉ dùng VnstockFetcher
        #   price_board trả về giá đầy đủ VND, không đi vào hàng đợi nguồn dữ liệu A-share
        # ----------------------------------------------------------
        if _is_vn_market(stock_code):
            quote = self._try_fetcher_quote(stock_code, "VnstockFetcher")
            if quote is not None:
                logger.info(f"[实时行情] 越南股 {stock_code} 成功获取 (来源: VnstockFetcher)")
                return self._enrich_realtime_quote(
                    quote,
                    realtime_cache_ttl=getattr(config, "realtime_cache_ttl", None),
                )
            if log_final_failure:
                logger.info(f"[实时行情] 越南股 {stock_code} 无可用数据源")
            return None

        # ----------------------------------------------------------
        # Cổ phiếu Mỹ (chỉ số + cá nhân) / HK — định tuyến hai nguồn chuyên dụng
        #   Sau khi cấu hình Longbridge: Longbridge ưu tiên, YFinance/AkShare bổ sung
        #   Chưa cấu hình Longbridge:   YFinance/AkShare ưu tiên, Longbridge bổ sung
        #   Chỉ số Mỹ:                  luôn ưu tiên YFinance (Longbridge không cung cấp báo giá chỉ số)
        # ----------------------------------------------------------
        ...
        if is_us or is_hk:
            ...
            # Cổ phiếu Mỹ (không phải chỉ số) thử bổ sung trường còn thiếu từ Finnhub/AlphaVantage
        ...
        # Lấy thứ tự ưu tiên nguồn dữ liệu đã cấu hình
        source_priority = [
            source.strip().lower()
        ...
        # Tất cả nguồn dữ liệu đều thất bại, trả về None (giảm cấp dự phòng)
        if log_final_failure:
            if errors:
    ...
    def get_chip_distribution(self, stock_code: str):
        ...
        # Nếu tính năng phân phối chip bị tắt, trả về None ngay
        ...
        candidate_fetchers = []
        # Duyệt trực tiếp danh sách nguồn dữ liệu đã được bộ quản lý sắp xếp theo priority
        for fetcher in self._get_fetchers_snapshot():
            # Chỉ xử lý các nguồn dữ liệu đã triển khai logic phân phối chip
            if not hasattr(fetcher, 'get_chip_distribution'):
                continue

            fetcher_name = fetcher.name
            # Tạo key circuit breaker động, ví dụ "TushareFetcher" -> "tushare_chip"
        ...
        for index, (fetcher, fetcher_name, source_key) in enumerate(candidate_fetchers):
            ...
            try:
                ...
                if _is_meaningful_chip_distribution(chip):
                    ...
                    # Kết quả rỗng hoặc placeholder: giải phóng slot thăm dò HALF_OPEN, tránh bị kẹt
    ...
    def get_stock_name(self, stock_code: str, allow_realtime: bool = True) -> Optional[str]:
        """
        Lấy tên cổ phiếu (tự động chuyển đổi nguồn dữ liệu)

        Thử lấy tên cổ phiếu từ nhiều nguồn:
        1. Kiểm tra cache trong bộ nhớ trước (nếu có)
        2. Thử bản đồ ánh xạ cục bộ và chỉ mục stocks.index.json
        3. Sau đó truy vấn báo giá thời gian thực nếu cần
        4. Lần lượt thử phương thức get_stock_name của từng nguồn dữ liệu
        ...
        Returns:
            Tên cổ phiếu, trả về None nếu tất cả nguồn đều thất bại
        """
        raw_stock_code = (stock_code or "").strip()
        # Normalize code (strip SH/SZ prefix etc.)
        stock_code = normalize_stock_code(stock_code)
        ...
        # 2. Thử lấy từ báo giá thời gian thực (nhanh nhất, có thể tắt theo nhu cầu)
        if allow_realtime:
        ...
        # 3. Lần lượt thử từng nguồn dữ liệu
        from .akshare_fetcher import _is_us_code
        ...
        # 4. Tất cả nguồn dữ liệu đều thất bại
    ...
    def batch_get_stock_names(self, stock_codes: List[str]) -> Dict[str, str]:
        """
        Lấy tên cổ phiếu hàng loạt

        Trước tiên thử lấy danh sách cổ phiếu từ nguồn dữ liệu hỗ trợ truy vấn hàng loạt,
        sau đó truy vấn từng cái cho những mã còn thiếu.

        Args:
            stock_codes: Danh sách mã cổ phiếu

        Returns:
            Dict {mã cổ phiếu: tên cổ phiếu}
        """
        result = {}
        ...
        # 2. Thử lấy danh sách cổ phiếu hàng loạt
        for fetcher in self._get_fetchers_snapshot():
    ...
    def get_main_indices(self, region: str = "cn") -> List[Dict[str, Any]]:
        """Lấy dữ liệu thời gian thực các chỉ số chính (tự động chuyển đổi nguồn dữ liệu)"""
        if region == "cn":
        ...
        if region == "vn":
            # Vietnam indices are served exclusively by VnstockFetcher; CN fetchers
            # ignore the region argument and would otherwise return A-share indices.
            for fetcher in self._fetchers:
                if type(fetcher).__name__ != "VnstockFetcher":
                    continue
                try:
            ...
            logger.warning("[MainIndices] region=vn 无可用越南指数数据源")
            return []
    ...
    def get_market_stats(self, *, purpose: str = "unspecified") -> Dict[str, Any]:
        """Lấy thống kê tăng/giảm thị trường (tự động chuyển đổi nguồn dữ liệu)"""
    ...
    def _build_vn_fundamental_context(
        self,
        stock_code: str,
        budget_seconds: Optional[float] = None,
    ) -> Dict[str, Any]:
        """Vietnam fundamental aggregation via vnstock (Company.overview + statements).

        Mirrors :meth:`_build_offshore_fundamental_context` shape/semantics but sources
        valuation/growth/earnings from VnstockFundamentalAdapter. A-share-only blocks
        (capital_flow, dragon_tiger, boards, institution) degrade to not_supported.
        """
        from src.config import get_config

        config = get_config()
        adapter = getattr(self, "_vnstock_fundamental_adapter", None)
        if adapter is None:
            return self._build_market_not_supported(
                market="vn", reason="VnstockFundamentalAdapter unavailable")
        ...
        # vnstock VCI is slower than the CN realtime APIs the global timeouts are tuned
        # for; give the VN bundle a generous floor so the concurrent overview+income+
        # balance fetch completes on a cold cache (env override: VN_FUNDAMENTAL_TIMEOUT).
        import os as _os
        _vn_floor = float(_os.getenv("VN_FUNDAMENTAL_TIMEOUT", "30") or "30")
        base_timeout = min(fetch_timeout, stage_timeout) if stage_timeout > 0 else 0.0
        bundle_timeout = max(base_timeout, _vn_floor)

        result_ctx: Dict[str, Any] = {
            "market": "vn",
            "valuation": {}, "growth": {}, "earnings": {},
            "institution": {}, "capital_flow": {}, "dragon_tiger": {}, "boards": {},
            "belong_boards": [], "coverage": {}, "source_chain": [], "errors": [],
        }

        if bundle_timeout <= 0:
            bundle, bundle_err, bundle_ms = {}, "fundamental stage timeout", 0
        else:
            bundle, bundle_err, bundle_ms = self._run_with_retry(
                lambda: adapter.get_fundamental_bundle(stock_code),
                bundle_timeout,
                "fundamental_bundle_vnstock",
            )
        if not isinstance(bundle, dict):
            bundle = {}

        extras = bundle.get("_valuation_extras", {})
        if not isinstance(extras, dict):
            extras = {}
        valuation_payload = {
            "pe_ratio": extras.get("pe_ratio"),
            "pb_ratio": extras.get("pb_ratio"),
            "total_mv": extras.get("total_mv"),
            "circ_mv": extras.get("circ_mv"),
            "high_52w": extras.get("high_52w"),
            "low_52w": extras.get("low_52w"),
            "foreign_ownership_pct": extras.get("foreign_ownership_pct"),
            "target_price": extras.get("target_price"),
        }
        chain = self._normalize_source_chain(
            bundle.get("source_chain", []),
            "fundamental_bundle_vnstock",
            str(bundle.get("status", "not_supported")),
            bundle_ms,
        )
        errs = list(bundle.get("errors", []))
        if bundle_err:
            errs.append(bundle_err)

        valuation_status = self._infer_block_status(
            {k: v for k, v in valuation_payload.items() if v is not None},
            "partial" if bundle else "not_supported",
        )
        result_ctx["valuation"] = self._build_fundamental_block(
            valuation_status, valuation_payload, chain, list(errs))

        growth_payload = bundle.get("growth", {}) if isinstance(bundle.get("growth"), dict) else {}
        earnings_payload = bundle.get("earnings", {}) if isinstance(bundle.get("earnings"), dict) else {}
        belong_boards = bundle.get("belong_boards", []) if isinstance(bundle.get("belong_boards"), list) else []

        growth_status = self._infer_block_status(growth_payload, str(bundle.get("status", "not_supported")))
        earnings_status = self._infer_block_status(earnings_payload, str(bundle.get("status", "not_supported")))
        result_ctx["growth"] = self._build_fundamental_block(growth_status, growth_payload, chain, list(errs))
        result_ctx["earnings"] = self._build_fundamental_block(earnings_status, earnings_payload, chain, list(errs))

        for block in ("institution", "capital_flow", "dragon_tiger", "boards"):
            result_ctx[block] = self._build_fundamental_block(
                "not_supported", {},
                [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
                ["not supported for VN market"],
            )
        result_ctx["belong_boards"] = belong_boards
        result_ctx["coverage"] = {
            "valuation": valuation_status, "growth": growth_status, "earnings": earnings_status,
            "institution": "not_supported", "capital_flow": "not_supported",
            "dragon_tiger": "not_supported", "boards": "not_supported",
        }
        for block in ("valuation", "growth", "earnings", "institution", "capital_flow", "dragon_tiger", "boards"):
            block_obj = result_ctx.get(block, {})
            if isinstance(block_obj, dict):
                result_ctx["errors"].extend(block_obj.get("errors", []) or [])
                result_ctx["source_chain"].extend(block_obj.get("source_chain", []) or [])

        active = (valuation_status, growth_status, earnings_status)
        if all(s == "not_supported" for s in active):
            result_ctx["status"] = "not_supported"
        elif "failed" in active or "partial" in active:
    ...
    def get_fundamental_context(
        ...
        if market == "vn":
            return self._build_vn_fundamental_context(
                stock_code,
    ...
    def get_capital_flow_context(self, stock_code: str, budget_seconds: Optional[float] = None) -> Dict[str, Any]:
        """Khối dữ liệu dòng tiền (fail-open)."""
        from src.config import get_config
    ...
    def get_dragon_tiger_context(self, stock_code: str, budget_seconds: Optional[float] = None) -> Dict[str, Any]:
        """Khối dữ liệu bảng dragon-tiger (fail-open)."""
        from src.config import get_config
    ...
    def get_board_context(self, stock_code: str, budget_seconds: Optional[float] = None) -> Dict[str, Any]:
        """Khối dữ liệu bảng xếp hạng ngành (fail-open)."""
        from src.config import get_config
    ...
    def _get_sector_rankings_with_meta(
            ...
            # Duyệt trực tiếp danh sách nguồn đã được bộ quản lý sắp xếp theo priority
            for fetcher in self._fetchers:
    ...
    def get_sector_rankings(self, n: int = 5) -> Tuple[List[Dict], List[Dict]]:
        """Lấy bảng xếp hạng tăng/giảm ngành (tự động chuyển đổi nguồn dữ liệu)"""
        # Thứ tự dự phòng cố định theo yêu cầu: Akshare(EM) -> Akshare(Sina) -> Tushare -> Efinance
    ...
    def get_concept_rankings(self, n: int = 5) -> Tuple[List[Dict], List[Dict]]:
        """Lấy bảng xếp hạng tăng/giảm nhóm/chủ đề (tự động chuyển đổi nguồn dữ liệu)."""
        last_error = ""
        for fetcher in self._fetchers:
            try:
    ...
    def get_hot_stocks(self, n: int = 10) -> List[Dict[str, Any]]:
        """Lấy bảng cổ phiếu được quan tâm nhất (tự động chuyển đổi nguồn dữ liệu)."""
        last_error = ""
        for fetcher in self._fetchers:
            try:
    ...
    def get_limit_up_pool(
        ...
        """Lấy danh sách cổ phiếu trần và chuỗi trần liên tiếp (tự động chuyển đổi nguồn dữ liệu)."""
        last_error = ""
        for fetcher in self._fetchers:
            try:
