...
def _is_vn_market(code: str) -> bool:
    """判定是否为越南股票（需显式 .VN 后缀，如 FPT.VN / VCB.VN）。

    越南个股代码为 2-3 个大写字母 + .VN 后缀。裸代码（FPT、GAS、BID）会与
    美股代码冲突，因此不在此识别 —— 必须显式带 .VN 后缀。
    """
    normalized = (code or "").strip().upper()
...
def _market_tag(code: str) -> str:
    """返回市场标签: cn/us/hk/jp/kr/vn."""
    if _is_vn_market(code):    # 越南 .VN 后缀优先判定，避免被美股正则吞掉
        return "vn"
...
class DataFetcherManager:
    ...
    def _init_default_fetchers(self) -> None:
        ...
        # 越南市场数据源（vnstock）。可选：未安装 vnstock 时跳过，不影响其它市场。
        try:
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        # ----------------------------------------------------------
        # 越南市场 (.VN 后缀) — 专用路由：仅 VnstockFetcher
        #   price_board 返回全量越盾价格，不走 A 股数据源队列
        # ----------------------------------------------------------
