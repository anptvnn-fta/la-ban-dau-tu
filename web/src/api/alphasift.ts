  ...
  async enable(): Promise<void> {
    ...
    try {
      ...
      if (!status.available) {
        ...
        throw new Error(`Lớp tích hợp AlphaSift không khả dụng${reason}. Vui lòng xác nhận backend đã cài đặt đủ phụ thuộc, nếu cần hãy chạy pip install -r requirements.txt hoặc build lại Docker/backend.`);
      }
    } catch (error) {
      try {
