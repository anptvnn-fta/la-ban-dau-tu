...
// Mặc định dùng API cùng origin, tránh trỏ nhầm sang localhost khi deploy production.
// Chỉ override khi VITE_API_URL được cấu hình tường minh.
...
const UNKNOWN_BUILD_TIME = 'Không xác định';
...
// Read version from package.json via Vite's define or env (optional).
// If neither is configured in vite.config.ts, fall back gracefully.
const runtimePackageVersion = (import.meta.env.VITE_APP_VERSION as string | undefined)?.trim()
  || PLACEHOLDER_WEB_VERSION;
const runtimeBuildTime = (import.meta.env.VITE_BUILD_TIME as string | undefined)?.trim() || '';
