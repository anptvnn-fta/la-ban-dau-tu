...
const STOCK_CODE_PATTERNS = [
  ...
  /^[A-Z]{2,3}\.VN$/, // Vietnam HOSE/HNX/UPCOM suffix format, for example FPT.VN
...
/**
 * Validate common A-share, HK, US, JP, KR, and VN stock code formats.
 */
export const validateStockCode = (value: string): ValidationResult => {
  const normalized = value.trim().toUpperCase();

  if (!normalized) {
    return { valid: false, message: 'Vui lòng nhập mã cổ phiếu', normalized };
  }
  ...
  return {
    valid,
    message: valid ? undefined : 'Định dạng mã cổ phiếu không hợp lệ',
    normalized,
  };
};
