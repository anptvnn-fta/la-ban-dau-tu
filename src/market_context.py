...
def detect_market(stock_code: Optional[str]) -> str:
    ...
    # Vietnam: explicit .VN suffix (e.g. FPT.VN). MUST precede the US branch,
    # because the US pattern r'^[A-Z]{1,5}(\.[A-Z]{1,2})?$' also matches 'FPT.VN'.
    # Kept consistent with data_provider.base._is_vn_market (2-3 letter base).
    if re.match(r'^[A-Z]{2,3}\.VN$', code):
        return "vn"
...
_MARKET_GUIDELINES = {
    ...
    "vn": {
        "zh": (
            "- 本次分析对象为 **越南股**（越南交易所 HOSE/HNX/UPCOM 上市股票，须带 `.VN` 后缀，如 `FPT.VN`）。\n"
            "- 请按越南市场语境分析：每日涨跌停幅度为 HOSE ±7% / HNX ±10% / UPCOM ±15%，T+2 结算，以越南盾(VND)计价；"
            "重点关注外资持股额度(foreign room，越南语「khối ngoại」)与外资买卖净额、越南国家银行(SBV)货币政策及宏观因素；"
            "不要套用 A 股涨跌停(±10%/±20%)、北向资金、龙虎榜、融资融券等 A 股专属概念。"
        ),
        "en": (
            "- This analysis covers a **Vietnam stock** (listed on HOSE/HNX/UPCOM, with a `.VN` suffix such as `FPT.VN`).\n"
            "- Use Vietnam-market context: daily price limits are HOSE ±7% / HNX ±10% / UPCOM ±15%, T+2 settlement, priced in VND; "
            "focus on foreign ownership room (\"khối ngoại\") and foreign net buy/sell flows, plus State Bank of Vietnam (SBV) policy and macro factors; "
            "do not apply China A-share concepts such as ±10%/±20% price-limit boards, Northbound flows, Dragon Tiger lists, or margin-financing narratives."
        ),
    },
}
