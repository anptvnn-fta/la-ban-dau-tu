...
def _is_vn_index_code(code: str) -> bool:
    """Return True for Vietnam stock codes with a .VN suffix (e.g. FPT.VN, VCB.VN).

    VN equity symbols are 2-3 uppercase letters; kept consistent with
    data_provider.base._is_vn_market and src.market_context.detect_market.
    """
    normalized = str(code or "").strip().upper()
    if "." not in normalized:
        return False
    base, suffix = normalized.rsplit(".", 1)
    if suffix == "VN":
        return base.isalpha() and 2 <= len(base) <= 3
    return False
...
def _build_stock_code_lookup(raw_items: list) -> Dict[str, str]:
    ...
    for item in raw_items:
        ...
        if not canonical_code:
            continue
        if not (_is_jp_kr_index_code(canonical_code) or _is_vn_index_code(canonical_code)):
            continue
        ...
        if "." in canonical_upper:
            ...
            # Vietnam: intentionally NO bare-code registration. VN 3-letter tickers
            # (FPT, GAS, BID) collide with real US tickers, so bare input must stay
            # US and VN is addressed only via the explicit .VN suffix (e.g. FPT.VN).
            # Exact-code + name/alias lookup for VN still work via exact_lookup above.
