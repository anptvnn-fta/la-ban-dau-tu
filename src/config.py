...
class Config:
    ...
    email_sender_name: str = "La Bàn Đầu Tư"  # Tên hiển thị người gửi
    ...
    # 大盘复盘市场区域：cn(A股)、hk(港股)、us(美股)、vn(越南股市)、both(cn/hk/us 三市场)
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            email_sender_name=os.getenv('EMAIL_SENDER_NAME', 'La Bàn Đầu Tư'),
    ...
    def _parse_report_language(cls, value: Optional[str]) -> str:
        ...
        if raw and not is_supported_report_language_value(raw):
            logging.getLogger(__name__).warning(
                "REPORT_LANGUAGE '%s' invalid, fallback to 'zh' (valid: vi/en/zh)",
                value,
            )
        return normalized
