...
class NotificationService(
    ...
    def save_report_to_file(
        ...
        # VN reports: deterministic post-render cleanup so any residual Chinese
        # scaffold label is replaced with the standard Vietnamese term. Gated on
        # a ".VN" ticker so cn/hk/us reports are never touched.
        if content and ".VN" in content:
            try:
                from src.vn_report_glossary import apply_vn_report_glossary
                content = apply_vn_report_glossary(content)
            except Exception as _vn_glo_exc:
                logger.debug(f"VN glossary post-render skipped: {_vn_glo_exc}")
