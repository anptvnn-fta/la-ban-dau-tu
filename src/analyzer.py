...
"""
    ...
    def _get_analysis_system_prompt(self, report_language: str, stock_code: str = "") -> str:
        ...
        # Vietnam stocks: force proper Vietnamese output regardless of REPORT_LANGUAGE.
        try:
            from src.market_context import detect_market as _detect_market
            _is_vn = _detect_market(stock_code) == "vn"
        except Exception:
            _is_vn = False
        if _is_vn:
            return base_prompt + """
...
- Giữ nguyên TẤT CẢ tên khóa JSON. `decision_type` phải giữ giá trị `buy|hold|sell`.
- TẤT CẢ văn bản hiển thị cho người dùng PHẢI viết bằng **tiếng Việt** chuẩn ngành chứng khoán Việt Nam. TUYỆT ĐỐI KHÔNG dùng tiếng Trung (chữ Hán), không pinyin, không trộn ngôn ngữ.
- Áp dụng cho: `stock_name`, `trend_prediction`, `operation_advice`, `confidence_level`, mọi nội dung dashboard, checklist và toàn bộ phần diễn giải/tóm tắt.
- Dùng ĐÚNG thuật ngữ chuẩn (KHÔNG dịch máy móc từ tiếng Trung):
  • Khuyến nghị: Mua mạnh / Mua / Nắm giữ / Quan sát / Giảm tỷ trọng / Bán / Bán mạnh.
  • Xu hướng: Tích cực / Tiêu cực / Đi ngang; "MA xếp tăng" (MA5>MA10>MA20) / "MA xếp giảm" (MA5<MA10<MA20).
  • Giá: Đóng cửa, Mở cửa, Cao nhất, Thấp nhất, Tham chiếu; Khối lượng (KL, đơn vị cp), Giá trị giao dịch (GTGD).
  • Biên độ trần/sàn: HOSE ±7%, HNX ±10%, UPCOM ±15%; thanh toán T+2.
  • Dòng vốn ngoại: "khối ngoại", "mua ròng"/"bán ròng", "room ngoại" (TUYỆT ĐỐI không dùng khái niệm A-share: 北向资金, 龙虎榜, 融资融券).
  • Kỹ thuật: Ngưỡng hỗ trợ / Ngưỡng kháng cự, Cắt lỗ, Giá mục tiêu, Độ lệch MA (BIAS), Quá mua/Quá bán, Cắt lên/Cắt xuống.
  • Cơ bản: Vốn hóa, Doanh thu, LNST cổ đông công ty mẹ, Biên lợi nhuận gộp, so với cùng kỳ (YoY).
  • GIỮ NGUYÊN thuật ngữ quốc tế: RSI, MACD, ADX, Bollinger, ATR, P/E, P/B, ROE, ROA, MA.
- Số tiền theo VND, dùng "tỷ"/"nghìn tỷ" (vd "120,6 nghìn tỷ VND"); ngày theo dd/mm/yyyy.
"""
        if lang == "en":
            return base_prompt + """
...
"""
        ...
        # 添加越南外资动向（khối ngoại） — VN market only
        vn_foreign = context.get("vn_foreign") if isinstance(context, dict) else None
        if isinstance(vn_foreign, dict) and vn_foreign:
            net_vol = vn_foreign.get("net_volume")
            net_val = vn_foreign.get("net_value_vnd")
            buy_vol = vn_foreign.get("buy_volume")
            sell_vol = vn_foreign.get("sell_volume")
            room_pct = vn_foreign.get("room_pct")
            owned_pct = vn_foreign.get("owned_pct")
            current_room = vn_foreign.get("current_room")
            trading_date = vn_foreign.get("trading_date", "N/A")
            ...
            def _fmt_shares(v):
                ...
                try:
                    v = int(v)
                    if abs(v) >= 1_000_000:
                        return f"{v / 1_000_000:.2f}M cp"
                    if abs(v) >= 1_000:
                        return f"{v / 1_000:.1f}K cp"
                    return f"{v} cp"
                except Exception:
                    return str(v)
            ...
            try:
                net_dir = "MUA RÒNG" if (float(net_vol or 0)) > 0 else ("BÁN RÒNG" if (float(net_vol or 0)) < 0 else "Trung lập")
            except Exception:
                net_dir = "N/A"

            prompt += f"""
### Khối Ngoại — Dòng Tiền Ngoại (外资动向) — {trading_date}
| 指标 | 数值 | 备注 |
|------|------|------|
| **净买入量** | **{_fmt_shares(net_vol)}** | {net_dir} |
| 净买入额 | {_fmt_vnd(net_val)} | |
| 外资买入量 | {_fmt_shares(buy_vol)} | |
| 外资卖出量 | {_fmt_shares(sell_vol)} | |
| 剩余外资额度 (room) | {_fmt_shares(current_room)} | 可继续买入的股数上限 |
| 剩余 room 比例 | {_fmt_pct(room_pct)} | 距外资持股上限的剩余空间 |
| 外资持股比例 | {_fmt_pct(owned_pct)} | 当前外资占总股本 |

> 外资净买入 + room 剩余少 → 买盘压力趋强；净卖出大 → 风险警示信号。
"""
...
"""
        ...
        # 添加越南市场扩展技术指标（vnstock_ta） — VN market only
        vn_ta = context.get("vn_ta_indicators") if isinstance(context, dict) else None
        if isinstance(vn_ta, dict) and vn_ta:
            prompt += f"""
### 越南市场扩展技术指标（vnstock_ta）
| 指标 | 数值 | 解读 |
|------|------|------|
| **RSI(14)** | **{vn_ta.get('rsi_14', 'N/A')}** | {vn_ta.get('rsi_zone', '')} |
| MACD线 (DIF) | {vn_ta.get('macd_line', 'N/A')} | {vn_ta.get('macd_cross', '')} |
| MACD信号线 (DEA) | {vn_ta.get('macd_signal', 'N/A')} | |
| MACD柱 (hist) | {vn_ta.get('macd_hist', 'N/A')} | 正值动能增强，负值动能减弱 |
| 布林上轨 (20,2) | {vn_ta.get('bb_upper', 'N/A')} VND | |
| 布林中轨 | {vn_ta.get('bb_mid', 'N/A')} VND | |
| 布林下轨 | {vn_ta.get('bb_lower', 'N/A')} VND | |
| **布林 %B** | **{vn_ta.get('bb_pct_b', 'N/A')}** | {vn_ta.get('bb_position', '')} |
| **ADX(14)** | **{vn_ta.get('adx_14', 'N/A')}** | {vn_ta.get('adx_trend_strength', '')} |
| ADX方向 | {vn_ta.get('adx_direction', 'N/A')} | +DI>-DI 为多头主导 |
| ATR(14) | {vn_ta.get('atr_14', 'N/A')} VND | 日均波幅约 {vn_ta.get('atr_pct', 'N/A')}%（止损参考） |

> RSI>70 超买 / RSI<30 超卖；%B>1 触上轨 / %B<0 触下轨；ADX>25 趋势有效；建议 1.5×ATR% 设置动态止损。
"""
...
请输出完整的 JSON 格式决策仪表盘。"""
        ...
        # Detect Vietnam stock from the context to force Vietnamese output.
        _vn_code = str((context or {}).get("stock_code") or (context or {}).get("code") or "")
        try:
            from src.market_context import detect_market as _dm
            _is_vn = (_dm(_vn_code) == "vn") or bool(
                (context or {}).get("vn_ta_indicators") or (context or {}).get("vn_foreign"))
        except Exception:
            _is_vn = False
...
- Giữ nguyên tất cả tên khóa JSON, không dịch khóa.
- `decision_type` phải giữ giá trị `buy`, `hold`, hoặc `sell`.
- TẤT CẢ giá trị văn bản hiển thị cho người dùng PHẢI viết 100% bằng tiếng Việt chuẩn ngành chứng khoán — TUYỆT ĐỐI không dùng tiếng Trung (chữ Hán), không pinyin, không trộn ngôn ngữ.
- Áp dụng cho stock_name, trend_prediction, operation_advice, confidence_level, mọi nội dung dashboard, checklist và toàn bộ phần diễn giải.
- Thuật ngữ chuẩn: Mua mạnh/Mua/Nắm giữ/Quan sát/Giảm tỷ trọng/Bán/Bán mạnh; Ngưỡng hỗ trợ/Ngưỡng kháng cự/Ngưỡng cắt lỗ/Giá mục tiêu; Khối ngoại, Mua ròng/Bán ròng, Room ngoại; Vốn hóa, Doanh thu, LNST. Giữ nguyên RSI/MACD/ADX/Bollinger/P/E/P/B/ROE/MA.
- Khi thiếu dữ liệu, ghi “{no_data_text}, không thể đánh giá”.
"""
