...
def _parse_dataframe(df: pd.DataFrame) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    for _, row in df.iterrows():
        ...
        if not code and name_val:
            code = resolve_name_to_code(name_val)
            if not code:
                logger.debug(f"[ImportParser] Không thể tra cứu mã từ tên: {name_val}")
...
def parse_import_from_bytes(data: bytes, filename: Optional[str] = None) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(data) > MAX_FILE_BYTES:
        raise ValueError(f"Tệp vượt quá giới hạn {MAX_FILE_BYTES // (1024 * 1024)}MB")

    ext = ""
    if filename:
        ext = "." + filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    logger.debug(f"[ImportParser] Bắt đầu phân tích tệp: filename={filename or '-'}, ext={ext or '-'}, bytes={len(data)}")
    ...
    if ext == ".xlsx" or looks_like_zip:
        try:
            ...
            if looks_like_zip:
                hint = (
                    "Hãy xác nhận: (1) tệp ở định dạng .xlsx; (2) sheet không rỗng; (3) tệp không bị hỏng. "
                    "Nếu là .xls, hãy lưu lại dưới dạng .xlsx rồi thử lại."
                )
                raise ValueError(f"Lỗi phân tích Excel: {e}. {hint}") from e
            # For extension-only mismatch (e.g. csv named .xlsx), fallback to text parsing.
            logger.warning(f"Phần mở rộng .xlsx nhưng không phân tích được Excel, chuyển sang đọc văn bản: {e}")

    # .xls not supported
    if ext == ".xls":
        raise ValueError("Chỉ hỗ trợ định dạng .xlsx, hãy lưu lại tệp .xls dưới dạng .xlsx rồi thử lại")
    ...
    for encoding in ("utf-8", "gbk"):
        ...
        raise ValueError("Không nhận dạng được mã hóa tệp, hãy dùng UTF-8 hoặc GBK")
    ...
    try:
        ...
        raise ValueError(
            f"Lỗi phân tích CSV: hãy kiểm tra dấu phân cách có nhất quán và số cột có khớp nhau không. "
            f"Nguyên nhân thường gặp: dấu nháy chưa đóng, số cột trong một hàng khác với các hàng còn lại. Lỗi gốc: {e}"
        ) from e
    except Exception:
        pass
...
def parse_import_from_text(text: str) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(text.encode("utf-8")) > MAX_TEXT_BYTES:
        raise ValueError(f"Văn bản vượt quá giới hạn {MAX_TEXT_BYTES // 1024}KB")

    logger.debug(f"[ImportParser] Bắt đầu phân tích văn bản dán: bytes={len(text.encode('utf-8'))}")
