...
class MarketStrategyBlueprint:
    ...
    def to_markdown_block(self) -> str:
        ...
        if self.region == "us":
            ...
            section_title = "### 6. Khung chiến lược"
        else:
...
VN_BLUEPRINT = MarketStrategyBlueprint(
    region="vn",
    title="Chiến lược tổng kết thị trường Việt Nam ba bước",
    positioning="Tập trung vào xu hướng chỉ số, dòng tiền (đặc biệt là khối ngoại) và luân chuyển nhóm ngành để lập kế hoạch giao dịch cho phiên kế tiếp.",
    principles=[
        "Xem hướng chỉ số VN-Index/VN30 trước, sau đó tới cấu trúc thanh khoản, cuối cùng là độ bền của nhóm ngành dẫn dắt.",
        "Mọi kết luận phải quy về tỷ trọng, nhịp độ và hành động kiểm soát rủi ro.",
        "Chỉ dựa trên dữ liệu trong phiên và tin tức 3 ngày gần nhất, không suy diễn thông tin chưa kiểm chứng.",
    ],
    dimensions=[
        StrategyDimension(
            name="Cấu trúc xu hướng",
            objective="Xác định thị trường đang tăng, đi ngang hay phòng thủ.",
            checkpoints=[
                "VN-Index/VN30/HNX-Index có cùng chiều không",
                "Tăng kèm thanh khoản hay giảm khi thanh khoản co lại",
                "Các mốc hỗ trợ/kháng cự quan trọng có bị phá vỡ",
            ],
        ),
        StrategyDimension(
            name="Dòng tiền & tâm lý",
            objective="Nhận diện khẩu vị rủi ro ngắn hạn và nhiệt độ thị trường.",
            checkpoints=[
                "Khối ngoại mua/bán ròng theo hướng và quy mô nào",
                "Thanh khoản toàn thị trường mở rộng hay thu hẹp",
                "Cổ phiếu dẫn dắt có dấu hiệu phân hoá không",
            ],
        ),
        StrategyDimension(
            name="Nhóm ngành dẫn dắt",
            objective="Chắt lọc dòng dẫn dắt có thể giao dịch và vùng cần tránh.",
            checkpoints=[
                "Ngân hàng/bất động sản/chứng khoán có catalyst rõ ràng không",
                "Nhóm ngành dẫn dắt có cổ phiếu đầu tàu kéo lên không",
                "Nhóm yếu có lan rộng không",
            ],
        ),
    ],
    action_framework=[
        "Tấn công: chỉ số đồng thuận tăng + thanh khoản mở rộng + dòng dẫn dắt mạnh lên.",
        "Cân bằng: chỉ số phân hoá hoặc đi ngang thanh khoản thấp, kiểm soát tỷ trọng và chờ xác nhận.",
        "Phòng thủ: chỉ số suy yếu + nhóm dẫn dắt giảm lan rộng, ưu tiên kiểm soát rủi ro và hạ tỷ trọng.",
    ],
)
