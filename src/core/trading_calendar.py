...
# Note: exchange-calendars (<=4.13) has NO Vietnam/HOSE calendar, so 'XHCM' will not
# resolve and VN falls through to the existing fail-open path (treated as a trading day).
MARKET_EXCHANGE = {"cn": "XSHG", "hk": "XHKG", "us": "XNYS", "jp": "XTKS", "kr": "XKRX", "vn": "XHCM"}
...
_CLOSING_AUCTION_WINDOW_MINUTES = {"cn": 3, "hk": 10, "us": 5, "vn": 15}
...
def get_market_for_stock(code: str) -> Optional[str]:
    ...
    Returns:
        'cn' | 'hk' | 'us' | 'jp' | 'kr' | 'vn' | None (None = unrecognized, fail-open: treat as open)
    """
    if not code or not isinstance(code, str):
        return None
    code = (code or "").strip().upper()

    # Vietnam: explicit .VN suffix (e.g. FPT.VN). Checked first for consistency with
    # data_provider.base._market_tag and src.market_context.detect_market.
    if code.endswith(".VN"):
        base = code[:-3]
...
def compute_effective_region(
    ...
    """
    if config_region not in ("cn", "hk", "us", "vn", "both"):
        config_region = "cn"
    if config_region in ("cn", "hk", "us", "vn"):
